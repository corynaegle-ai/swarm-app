...
export class Pipeline {
  ...
  private async executePipeline(
    ...
    try {
      ...
      // Stage 5: Finalize
      await this.stageFinalize(deploymentId, manifest, 'success', undefined, trigger);
      ...
      await this.stageFinalize(deploymentId, manifest, 'failed', error.message, trigger);
    }
  }
  ...
  private async stageFinalize(
    ...
    // GAP #4: Update ticket state after successful deployment
    if (status === 'success' && trigger?.ticket_id) {
      try {
        const updated = await ticketApi.updateTicketState(trigger.ticket_id, 'done');
        if (updated) {
          logger.info('Ticket marked as done', { 
            ticketId: trigger.ticket_id, 
            deploymentId 
          });
        }
      } catch (ticketError) {
        // Log but don't fail deployment for ticket update failure
        logger.error('Failed to update ticket after deployment', { 
          ticketId: trigger.ticket_id, 
