...
import { queueExecution } from '../engine/queue.js';

const router = Router();

router.get('/', async (req, res) => {
  try {
    const result = await db.query(`SELECT id, name, description, active, created_at, updated_at FROM flows ORDER BY updated_at DESC`);
...
router.post('/', async (req, res) => {
  try {
    ...
    const id = uuidv4();
    await db.query(`INSERT INTO flows (id, name, description, definition, active) VALUES ($1, $2, $3, $4, FALSE)`,
...
router.post('/:id/execute', async (req, res) => {
  try {
    ...
    const execId = uuidv4();
    await db.query(`INSERT INTO executions (id, flow_id, status, trigger_data) VALUES ($1, $2, 'pending', $3)`,
      [execId, req.params.id, JSON.stringify(req.body.triggerData || {})]);
    await queueExecution(execId, req.params.id);
...
router.get('/:id/runs', async (req, res) => {
  try {
    const result = await db.query(`SELECT id, status, started_at, completed_at, error FROM executions WHERE flow_id = $1 ORDER BY started_at DESC LIMIT 50`, [req.params.id]);
    res.json(result.rows);
  } catch (err) {
    res.status(500).json({ error: (err as Error).message });
  }
