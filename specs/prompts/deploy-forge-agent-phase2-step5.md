# Deploy FORGE Agent - Phase 2: Continue from Step 5

## Prerequisites (Already Complete)
- [x] Steps 2.1-2.4: Engine CLI installed and dependencies ready
- [x] Step 2.5: Test ticket exists (`engine-test-001`)
- [x] SSH connected to droplet
- [x] PATH set: `export PATH=/usr/local/bin:/usr/bin:/bin:$PATH`
- [x] `swarm-cleanup` ran successfully
- [x] In directory `/opt/swarm/engine`
...
## Step 5: Verify Test Ticket Exists and is Ready

```bash
sqlite3 /opt/swarm-platform/data/swarm.db "SELECT id, state, assignee_type FROM tickets WHERE id='engine-test-001';"
```

**Expected output**: `engine-test-001|ready|agent`
...
## Success Criteria
- [ ] Test ticket verified ready (step 5)
